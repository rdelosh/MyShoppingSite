...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			PreparedStatement pstmt1 = myconn.prepareStatement("select * from user where email=?");
			...
			while(myrs.next()){
				...
				db_fname = myrs.getString("firstname");
				db_lname = myrs.getString("lastname");
			}
		...
		if(password.equalsIgnoreCase(db_password)){
			...
			myuserbean.setEmail(db_email);
			myuserbean.setFname(db_fname);
			myuserbean.setLname(db_lname);
